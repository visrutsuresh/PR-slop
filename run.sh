...
case "$1" in
  live)
    ...
    if [ -n "${3:-}" ]; then
      python3 src/live.py "$2" --pr "$3"
    else
      # No --limit here. src/live.py defaults to 100 and asks how deep to go,
      ...
      if [ -n "${PRSLOP_LIMIT:-}" ]; then
        python3 src/live.py "$2" --limit "$PRSLOP_LIMIT"
      else
        python3 src/live.py "$2"
      fi
    fi
    ;;
  triage)
  ...
  probe)
    ...
    python3 src/isolation_probe.py
    ;;
  baseline)
    if [ "${REGENERATE:-0}" = "1" ]; then
      python3 src/isolation_probe.py
      python3 src/run_baseline.py --generate
    else
      echo "[baseline] replaying committed responses, no network, no credential"
      python3 src/run_baseline.py --replay
    fi
    ;;
  script)
    ...
    if [ "${REGENERATE:-0}" = "1" ]; then
      python3 src/isolation_probe.py
      python3 src/run_advanced.py --generate
    else
      echo "[script] two-stage version, replaying committed responses"
      python3 src/run_advanced.py --replay
    fi
    ;;
  advanced|agent)
    # THE SHIPPED SYSTEM. Four roles with a loop. See IMPROVEMENT-CHANGELOG.md for the six
    # versions and why v4 is the one that ships.
    if [ "${REGENERATE:-0}" = "1" ]; then
      python3 src/isolation_probe.py
      PRSLOP_AGENT_DIR=data/responses/agent-v4 python3 src/agent_v4.py --generate
    else
      echo "[agent] shipped system, replaying committed responses, no credential"
      PRSLOP_AGENT_DIR=data/responses/agent-v4 python3 src/agent_v4.py --replay
    fi
    ;;
  mcp-install)
    ...
    printf '{\n  "mcpServers": {\n    "pr-slop": {\n      "command": "python3",\n      "args": ["%s/src/mcp_server.py"]\n    }\n  }\n}\n' "$here" > .mcp.json
    echo "[mcp] .mcp.json now points at $here/src/mcp_server.py"
    echo "[mcp] restart your assistant, or: claude mcp add pr-slop -- python3 $here/src/mcp_server.py"
    ;;
  mcp)
    ...
    echo "[mcp]   claude mcp add pr-slop -- python3 $(cd "$(dirname "$0")" && pwd)/src/mcp_server.py" >&2
  ...
  evidence)
    ...
    ;;
  versions)
    # rebuilds the six-version table in IMPROVEMENT-CHANGELOG.md from committed answers,
