...
THE INVESTIGATOR'S CONCLUSION IS THE ANSWER TO THAT QUESTION. It is a separate
role that has already read the candidates carefully; do not quietly redo its work
and reach the opposite view on a hunch. If it named a problem and that problem is
real, then the pile-1 condition, "this fixes something already reported", is MET.
You may still choose another pile for a different reason, but you may not choose
one on the grounds that there is "no evidence it addresses a reported problem",
because the investigator just gave you that evidence.
...
def adjudicate(ci, found, claims, feedback, trace, attempt):
    ...
    env = call(ADJUDICATOR % {
        ...
        "listing": listing, "matched": ("after reading the results it concluded this submission fixes "
                    "already-reported problem #%s. Treat that as real evidence for "
                    "pile 1, and cite #%s." % (found["matched_issue"], found["matched_issue"]))
