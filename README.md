...
**What "USD" means here, and what it does not.** Every cost in this repository is a *model-usage estimate*, priced at published per-token rates for the model the tool calls. It is not a figure read off a bill. `src/agent_v4.py:60` invokes `claude -p`, so the model runs through whatever account the machine is already signed in to. On an API key that estimate approximates a real charge. **On a Claude subscription it consumes your plan's usage, and nothing appears on a card at all.** The number is still the right thing to show a maintainer, because usage is the real constraint either way, and because a tool that can spend eight hundred dollars of anything should say so before it does. But if you reproduce this on a subscription and go looking for the charge, you will not find one, and that is expected rather than a discrepancy.
