...
- [x] **4. Agent trajectories.** 45 records in `traces/`, covering all three systems. 91 files are tracked: 45 rendered pages, 45 machine-readable JSONL logs, and `traces/INDEX.md`. The shipped agent's 15 are marked **captured**, written as each run happened. The other 30 are marked **reconstructed** from committed responses. `traces/INDEX.md` lists them.
...
- [x] **Judge access. THIS IS THE ONE THAT DISQUALIFIES YOU.** The repository is **PUBLIC**, verified with `gh repo view visrutsuresh/PR-slop --json isPrivate`, which returns `false`. Rule 10 requires judges to be able to run it, and a private repository is thrown out before anyone reads a line of the work. Re-check this after the final push.
...
6. Open `reports/example-microsoft-vscode.html` in the fresh clone. This is the live tool's output, committed on purpose, because `./run.sh live` needs a GitHub login and real spend, roughly a few USD for a small queue. The footer of that page reports 4.10 USD for the 9 pull requests in this particular run. That figure moves with queue size and prices, so read it off the page rather than trusting a number quoted here.
