...
- [x] **4. Agent trajectories.** 45 records in `traces/`, 91 files tracked once the machine-readable JSONL is counted alongside each rendered page,, covering all three systems. The shipped agent's 15 are marked **captured**, written as each run happened. The other 30 are marked **reconstructed** from committed responses. `traces/INDEX.md` lists them.
