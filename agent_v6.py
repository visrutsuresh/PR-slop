...
RESPONSE_DIR = os.environ.get("PRSLOP_AGENT_DIR", "data/responses/agent")
...
Judge them. If one is plausibly the problem this submission fixes, say so and stop.

Do not hold out for a perfect match. A report written by a user and a fix written
by a developer describe the same thing in different words, so an exact overlap is
rare and is not the bar. If a candidate is about the same behaviour in the same
area, that counts.

Only search again if nothing in the list is even in the right area.

Say how sure you are, honestly:
  "certain"  - this is plainly the same problem, the submission clearly fixes it
  "likely"   - same area and same behaviour, but you are inferring
  "guess"    - it is the closest thing on the list, nothing more

Reply as JSON:
{"decision": "stop" or "search_again",
 "matched_issue": 12345 or null,
 "certainty": "certain" or "likely" or "guess",
 "why": "one sentence",
...
def investigate(ci, search, trace):
    ...
    for rnd in range(1, MAX_SEARCH_ROUNDS + 1):
        ...
        trace.append({"role": "investigator", "action": "judged the results",
                      ...
                      "why": v.get("why"), "certainty": v.get("certainty"),
                      "cost": env2.get("total_cost_usd")})
        matched = v.get("matched_issue")
        certainty = v.get("certainty") or "guess"
        if v.get("decision") != "search_again" or rnd == MAX_SEARCH_ROUNDS:
            return {"query": query, "hits": hits, "matched_issue": matched,
                    "certainty": certainty, "rounds": rnd}
...
ITS CONCLUSION: %(matched)s

HOW MUCH WEIGHT TO GIVE THAT depends on how sure the investigator said it was:

  certain  - treat the pile-1 condition as MET. Do not quietly redo its work and
             reach the opposite view on a hunch. It is a separate role that read
             the candidates properly.
  likely   - real supporting evidence, but not settled. Weigh it with everything
             else.
  guess    - it is only the nearest thing on a list. Nearly every submission has
             a nearest thing. Treat it as almost no evidence at all.

--- what the claim checker found, after reading the real source ---
Does the code match what the submission SAYS it does: %(supported)s
Reason: %(why)s

READ THAT CORRECTLY. It tells you whether the description matches the code. It
tells you NOTHING about whether maintainers accepted the work. Those are
different questions and confusing them is the single biggest mistake you can
make here.

A "no" from the claim checker is NOT grounds for pile 3, and it is NOT grounds
for overturning the investigator either. The two roles answer different questions:
one asks whether the description matches the code, the other asks whether this
fixes something already reported. A "no" on the first says nothing about the
second.

A "no" from the claim checker is NOT grounds for pile 3. Real, wanted work is
often described loosely, and maintainers merge it anyway. Do not put something
in pile 3 because you judge the code to be poor, or the description sloppy, or
the change small.

Also know what pile 3 actually looks like on this project. It is mostly NOT bad
work. It is mostly automated housekeeping, or the team's own work closed because
they took a different approach, or something superseded. Put a submission in
pile 3 when it looks like one of THOSE, or like machine-written filler with no
real substance. Not when you simply dislike the code.
%(feedback)s
You must put this in pile 1, 2 or 3. Do NOT answer 0 here. A triage tool that
shrugs leaves the maintainer exactly where they started, reading everything. If
you are unsure, still choose the most likely pile and mark your confidence low.

Before you answer, know the base rate, because it is strongly lopsided and a
real maintainer knows it: on this project roughly NINE IN TEN closed submissions
were merged. Merged is the common case by a wide margin. Pile 3 is the exception
and needs a positive reason, not merely an absence of enthusiasm.
...
def adjudicate(ci, found, claims, feedback, trace, attempt):
    ...
    env = call(ADJUDICATOR % {
        ...
        "listing": listing, "matched": ("it concluded this submission fixes already-reported problem #%s, and "
                    "it rated its own certainty as %s. Cite #%s if you use it."
                    % (found["matched_issue"], found.get("certainty", "guess"), found["matched_issue"]))
                   if found.get("matched_issue") else
                   "it found no already-reported problem that this clearly fixes. That is "
                   "weak evidence, not proof: the search is imperfect and may simply have "
                   "missed it. Do NOT treat a failed search as a reason for pile 3.",
